...
test("selects EDT morning and update deliveries", () => {
  assert.equal(selectDelivery("35 12 * * 1-5", Date.parse("2026-08-28T12:35:00Z")), "morning");
  assert.equal(selectDelivery("12 18 * * 1-5", Date.parse("2026-08-28T18:12:00Z")), "update");
});

test("selects EST morning and update deliveries", () => {
  assert.equal(selectDelivery("35 13 * * 1-5", Date.parse("2026-12-04T13:35:00Z")), "morning");
  assert.equal(selectDelivery("12 19 * * 1-5", Date.parse("2026-12-04T19:12:00Z")), "update");
});

test("skips the inactive daylight-saving counterpart", () => {
  assert.equal(selectDelivery("5 14 * * 1-5", Date.parse("2026-08-28T14:05:00Z")), null);
  assert.equal(selectDelivery("5 13 * * 1-5", Date.parse("2026-12-04T13:05:00Z")), null);
});
